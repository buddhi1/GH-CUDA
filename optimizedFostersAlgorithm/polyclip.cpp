...
// compile:
//
// g++ -std=c++11 polyclip.cpp -o polyclip
//
// usage:
//
...
RelativePositionType oracle(vertex* Q, vertex* P1, vertex* P2, vertex* P3) {
  ...
  // cout << Q->p << ", P1(" << P1->p << ") P2(" << P2->p <<  ") P3(" << P3->p << ") " << s1 << " " << s2 << " " << s3 << endl;
...
void labelIntersections() {
  ...
  for (polygon& P : PP) 
    for (vertex* I : P.vertices(INTERSECTION)) {
      ...
      // cout<< "qm " << Q_m_type << " " << I->p << endl;
  		RelativePositionType Q_p_type = oracle(Q_p, P_m, I, P_p);
      // cout<< "qp " << Q_p_type << " " << I->p << endl;
        ...
        count[1]++;
  		...
  		if ( ( (Q_m_type == IS_P_m) && (Q_p_type == LEFT) ) ||
  			...
  			I->label = ON_RIGHT;
      cout<< "ini label " << I->p << " " << I->label << endl;
  	}
  ...
  for (int i=0; i<2; ++i) {
    ...
    for (polygon& P : *P_or_Q) {
      ...
      for (vertex* I : P.vertices(INTERSECTION, V)) {
        ...
        cout<< I->p << " & " << I->enex << endl;
  		}
      cout<< V->p << " *& " << V->enex << endl;
    }  
  }
  cout << "\n\n--------------------------------\n"; 
...
int main(int argc, char* argv[])
{
  ...
  //   cout << V->p.x << ", " << V->p.y << " " << V->source << endl;
  // }
  // cout << "QQ" << endl;
